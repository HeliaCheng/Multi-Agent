...
def create_agents_and_tasks(num_agents: int, num_tasks: int, WorldInfoInput: WorldInfo, config_data):
    ...
    # agent type
    agent_quad_default.agent_type = config_data["AGENT_TYPES"].index("Robot")
    ...
    # agent type
    agent_car_default.agent_type = config_data["AGENT_TYPES"].index("Robot")
    # agent cruise velocity (m/s)
    ...
    # task type
    task_track_default.task_type = config_data["TASK_TYPES"].index("Delivery")
    # task reward
...
def create_set(num_agents: int, num_tasks: int, WorldInfoInput: WorldInfo, config_data):
    agent_default = Agent()
    # agent type
    agent_default.agent_type = config_data["AGENT_TYPES"].index("Robot")
    # agent cruise velocity (m/s)
    agent_default.nom_velocity = float(config_data["AGENT_DEFAULT"]["NOM_VELOCITY"])
    # # agent fuel penalty (per meter)
    # agent_quad_default.fuel = float(config_data["QUAD_DEFAULT"]["FUEL"])
    ...
    task_default = Task()
    # task type
    task_default.task_type = config_data["TASK_TYPES"].index("Delivery")
    # task reward
    ...
    num_agents = 5
    print(num_tasks)
    PList = list(range(1, num_tasks + 1))
    print(PList)
    # create random agents
    for idx_agent in range(0, num_agents):
        # create a new instance of dataclass agent_quad_default
        ...
        AgentList[idx_agent].agent_id = idx_agent
        AgentList[idx_agent].x =3
        AgentList[idx_agent].y =-1+idx_agent*1
        AgentList[idx_agent].z = 0
    ...
    for idx_task in range(0, num_tasks):
        ...
        TaskList[idx_task].task_id = idx_task
        TaskList[idx_task].x = -2
        TaskList[idx_task].y = -1.5+idx_task*0.5
        TaskList[idx_task].z = 0
        TaskList[idx_task].start_time =0
        TaskList[idx_task].end_time = 100
        TaskList[idx_task].P = PList[-idx_task-1]
        print("Task的优先级", TaskList[idx_task].P)
    for n in range(num_tasks):
        print("Task " + str(n))
        print("Task"+str(n)+"的坐标xyz:"+str(+TaskList[n].x) + ", " + str(TaskList[n].y) + ", " + str(TaskList[n].z))
        print("Task"+str(n)+"的start,end time:"+str(TaskList[n].start_time) + " - " + str(TaskList[n].end_time))
    for m in range(num_agents):
        print("Agent " + str(m))
        print("Agent"+str(m)+"的坐标xyz:"+str(AgentList[m].x) + ", " + str(AgentList[m].y) + ", " + str(AgentList[m].z))
