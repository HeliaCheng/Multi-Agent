...
#CBBA/consensus-based bundle algorithm 复现by_cwr
# 1.算法，2.自动生成模拟环境 3.可视化
##实验针对什么？？？？？
...
class CBBA():
        ...
        if (self.AgentList[idx_agent].agent_type == self.agent_types.index("quad")) or \
            ...
            if not task_prev:
                ...
                dt = math.sqrt((self.AgentList[idx_agent].x - task_current.x) ** 2 +
                               ...
                               (self.AgentList[idx_agent].z - task_current.z) ** 2) / self.AgentList[
                         idx_agent].nom_velocity
    ...
    def plot_assignment(self):
        ...
        for m in range(self.num_tasks):
            ...
            ax_3d.text(self.TaskList[m].x + offset, self.TaskList[m].y + offset, self.TaskList[m].start_time,
                       "T" + str(m))
        ...
        def f(marker_type, color_type):
            return plt.plot([], [], marker=marker_type, color=color_type, ls="none")[0]
...
#     Able = True
#     # Xi
#     Task10 = np.zeros(1, Ntc)
#     # Yi
#     WinBid = np.zeros(1, Ntc)
#     # Ci
#     Score = np.zeros(1, Ntc)
#     # Gi
#     G = np.zeros(1, Ntc)
#     # Zi
#     WinAgent = np.zeros(1, Ntc)
#     # Neighbors
#     neighbours = np.zeros(1, Nrc)
...
#
#
# RobotSet=[Robot(i) for i in range(Nr)]
# TaskSet=[Task(i) for i in range(Nt)]
#
#
# print(RobotSet[0].Seq)
# print(RobotSet)
